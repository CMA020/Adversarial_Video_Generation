                ...
                with tf.name_scope('scale_' + str(scale_num)):
                    ...
                    with tf.name_scope('calculation'):
                        def calculate(height, width, inputs, gts, last_gen_frames):
                            ...
                            inputs = tf.image.resize(inputs, [scale_height, scale_width])
                            scale_gts = tf.image.resize(gts, [scale_height, scale_width])

                            # for all scales but the first, add the frame generated by the last
                            # scale to the input
                            if scale_num > 0:
                                last_gen_frames = tf.image.resize(
                        ...
                        if c.ADVERSARIAL:
                            self.d_scale_preds.append(tf1.placeholder(tf.float32, [None, 1]))
